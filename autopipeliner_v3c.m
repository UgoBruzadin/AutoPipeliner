...
classdef autopipeliner_v3c
    methods(Static)
        
        %autopipeliner_v3c = dbstack;
        %autopipeliner_v3c = st.name;

        function [batchFolder, OGFolder] = newBatches(Batches,batchFolder,files,batchCounter) %store all batches to be done
            if nargin < 2
               batchFolder = cd; 
            end
            ...
            for i=1:length(Batches)
                ...
                [batchFolder] = autopipeliner_v3c.createBatchFolder(OGFolder,batchName,files); 
                %cd(OGFolder)
                % --- starts pipeline
                [batchFolder] = autopipeliner_v3c.pipeline(Batches(i),batchFolder,batchCounter); %start the pipeline
                % --- adds +1 to batch counter
                batchCounter = batchCounter+1; 
            end
        end
%         
%         function [batchFolder, OGFolder] = oldbatches(Batches,batchFolder,batchCounter,files) %store all batches to be done
%             if nargin < 4
%                 % --- move to OG folder
%                 cd(batchFolder);
%                 % --- gets files in OG folder
...
%         end
        ...
        function [batchFolder] = createBatchFolder(path,folderName,files,batchCounter) %works!!!
            ...
            if ~isfolder(strcat(path,folderName))
                mkdir (folderName)
                batchFolder = strcat(basefolder,'\', folderName);
                %restarted = 0;
            else
                batchFolder = strcat(path,folderName);
                %restarted = 1;
            end
            % --- this huge 'if' creates a list of the files selected
            % -- I wish it was smaller but this does the trick.
            %if nargin < 3
%             files = getNewFiles(basefolder,batchFolder,'*.set');
%             fdtfiles = getNewFiles(basefolder,batchFolder,'*.fdt');
%             %end
%             if ~isfile('done.txt')
%                 if ~isempty(files)
%                     % --- add: compare files
...
%                 end
%                 if ~isempty(fdtfiles)
%                     parfor j=1:length(fdtfiles)
%                         copyfile(fdtfiles(j).name, batchFolder)
%                     end
%                 end
%                 save('done.txt');
%             end
        ...
        function [filesFolder] = pipeline(Batch,batchFolder,batchCounter) %store the functions to be rolled in this batch
            % --- print
            ...
            for i=scriptCounter:length(Scripts) %for loop, loops the number of commands
                ...
                [filesFolder] = autopipeliner_v3c.Function(batchFolder,Scripts(i),filesFolder,scriptCounter,batchCounter);
                scriptCounter = scriptCounter + 1;%adds one folder to the counter
            end
        end
        
        function [files, filesPRE, filesEND] = createfolders(batchFolder,lastFolder,folderName,scriptCounter,batchCounter)
            %------ create the folders where the pipeline will run
            %------ filePRE = strcat(basefolder,'\', type, '\pre'); %copies files
            %------ one can I turn it off)
            ...
            folderList = dir();
            %if ~isfile('done.txt')
                %  --- lastFolder2 contains the real place where the files are
                %  ---- originally, either in the batch folder, or not.
            %    lastFolder2 = lastFolder;
            %else
            
            cd ..
            baseFolder = pwd;
            %end
            %previousFolderPath2 = previousFolderPath;
            %cd(lastFolder)
            cd(batchFolder);
            ...
            % --- checks if Pre folder already exists, if not, creates it.
            cd(folderName);
            filesEND = pwd;
            if ~isfolder('Pre')
            ...
            if scriptCounter == 1
                lastFolder = baseFolder;
            end
            files = getNewFiles(lastFolder,filesPRE,'*.set','*.set',pwd);
            fdtfiles = getNewFiles(lastFolder,filesPRE,'*.fdt','*.fdt',pwd);
            ...
            if ~isempty(fdtfiles)
                cd(lastFolder)
                parfor k=1:length(fdtfiles)
                    if scriptCounter == 1
                        copyfile(fdtfiles(k).name, filesPRE)
                    else
                        movefile(fdtfiles(k).name, filesPRE)    
                    end
                end
                cd(filesPRE)
            end
            
            % --- if this folder has been done, skips it.
            cd(filesEND)
            if ~isfile('done.txt')
                cd(filesPRE);
                files = getNewFiles(filesPRE,filesEND,'*.set','*.set',pwd);
            else
                cd(filesPRE);
                files = [];
            end
        ...
        function [filePOST] = Function(batchFolder,Scripts,filesFolder,scriptCounter,batchCounter) %magic function, runs the code asked!
            if nargin < 5
                batchCounter = 2;
            end
            if nargin < 4
            ...
            folderList = dir();
            %if isempty(folderList)
            [files, filePRE, filePOST] = autopipeliner_v3c.createfolders(batchFolder,filesFolder,folderNameDate,scriptCounter,batchCounter); %creates a folder for the pipeline
            %else
        ...
        function [filePOST] = splitFunction(batchFolder,Scripts,filesFolder,scriptCounter,batchCounter) %magic function, runs the code asked!
            if nargin < 5
                batchCounter = 2;
            end
            if nargin < 4
...
%             [files, filePRE, filePOST] = autopipeliner_v3c.createfolders(batchFolder,filesFolder,folderNameDate,counter); %creates a folder for the pipeline
%             % --- moves down to scripts new directory
%             cd(filePOST);
%             parfor i=1:length(files)
%                 % --- load EEG
...
%         end
        ...
        function [setList,fdtList] = comparefiles(pre,post)
            ...
            if size(subFolders,1) > 2
                ...
                %folder = autopipeliner_v3c.getLastFolder(folder);
                end
            else
...
%EEG =  pop_loadset(file1(i).name, FileInput1,  'all','all','all','all','auto');
        ...
        %figuregenerators!
            ...
            files = dir('*.set');
            autopipeliner_v3c.clean(); %wipe the memory
            t = datetime('now','TimeZone','local','Format','dMMMy-HH.mm'); %gets the datetime
            fname = strcat(mfilename,'.'); %get the name of this function for future use, adds a dot to it
            ...
            for i=1:length(files)
                ...
                %[individualReport] = autopipeliner_v3c.tempReport(files(i).name,EEG);%changedneedsfixing
                ...
                autopipeliner_v3c.fft(files(i),EEG);
                if ~isempty(EEG.icaweights)
                    autopipeliner_v3c.componentFigures(files(i),EEG) %fixed and added 2/3/2020
                end
                EEG = pop_delset(EEG,1); %fixed and added 2/3/2020
            end
            %makes report
            autopipeliner_v3c.report(strcat('finalReport',t)); %fixed and added 2/3/2020
            autopipeliner_v3c.emptyTrash(); %deletes binica's leftover trash
            autopipeliner_v3c.txt(strcat('processing of ', folderNameDate,' is over')); %fixed and added 2/3/2020
        end
    end
end