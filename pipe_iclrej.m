function [EEG, acronym] = pipe_iclrej(EEG, loop)
fprintf('ic label rejection loops')    
%------ pipeliner.icloop([.8],EEG) or pipeliner.icloop(.8,EEG)
    ...
    for j=1:loop
        ...
        EEG = pop_par_icflag(EEG, [NaN NaN;0.90 1;0.90 1;0.90 1;0.90 1;0.90 1;NaN NaN]);
        ...
        try EEG = pop_par_runica(EEG,'extended',1,'pca',IC,'verbose','off'); %does a PCA of IC
        catch EEG = pop__par_runica(EEG,'extended',1,'pca',IC,'verbose','off'); %BUG: it looks likes it picks up the wrong binica files. inconsistent error.
        end
