...
classdef autopipeliner_v1d
    methods(Static)
        
        %autopipeliner_v1d = dbstack;
        %autopipeliner_v1d = st.name;
        
        function [foldersList]= batches(batches,batchFolder,batchCounter) %store all batches to be done
            
            %make it return a list with all folders and paths to files
            %this way, it will be possible to perform comparisons between
            %files
            
            if nargin < 3
                %counter for batches. If not given, default = 1;
                batchCounter = 1;
            end
            % --- gets current folder
            OGFolder = batchFolder;
            
            % --- store folder in list
            foldersList = strings();
            foldersList(1) = batchfolder;
            
            % --- for loop for each batch to be run
            for i=1:length(batches)
                
                % --- move to OG folder & gets files in OG folder
                cd(OGFolder);
                files = dir('*.set');
                ...
                [batchFolder] = autopipeliner_v1d.createBatchFolder(OGFolder,files,batchName); 
                
                % --- returns to OG folder
                %cd(OGFolder) - only necessary
                
                % --- starts pipeline
                [batchFolder] = autopipeliner_v1d.pipeIn(batches(i),batchFolder); %start the pipeline
        ...
        function [filesFolder] = pipeIn(scripts,batchFolder) %store the functions to be rolled in this batch
            ...
            for i=scriptCounter:length(scripts) %for loop, loops the number of commands
                ...
                [filesFolder] = autopipeliner_v1d.Function(batchFolder,scripts(i),filesFolder,scriptCounter);
        ...
        function [filePOST] = Function(batchFolder,script,filesFolder,counter) %magic function, runs the code asked!
            ...
            autopipeliner_v1d.clean(); %wipe the memory
            ...
            [files, filePRE, filePOST] = autopipeliner_v1d.createfolders(batchFolder,filesFolder,folderNameDate,counter); %creates a folder for the pipeline
...
%                 end
            ...
            %autopipeliner_v1d.emptyTrash(); %deletes binica's leftover trash
            %sends text to me
            %autopipeliner_v1d.txt(strcat('processing of ', folderNameDate,' is over')); %fixed and added 2/3/2020
...
%EEG =  pop_loadset(file1(i).name, FileInput1,  'all','all','all','all','auto');
        ...
        function folder = getLastFolder(currentDirectory)
            ...
            if size(subFolders,1) > 2
                ...
                %folder = autopipeliner_v1d.getLastFolder(folder);
                end
            else
        ...
        %figuregenerators!
            ...
            for i=1:length(files)
                ...
                %[individualReport] = autopipeliner_v1d.tempReport(files(i).name,EEG);%changedneedsfixing
                ...
                autopipeliner_v1d.fft(files(i),EEG);
                if ~isempty(EEG.icaweights)
                    autopipeliner_v1d.componentFigures(files(i),EEG) %fixed and added 2/3/2020
                end
                EEG = pop_delset(EEG,1); %fixed and added 2/3/2020
            end
            %makes report
            autopipeliner_v1d.report(strcat('finalReport',t)); %fixed and added 2/3/2020
            autopipeliner_v1d.emptyTrash(); %deletes binica's leftover trash
            autopipeliner_v1d.txt(strcat('processing of ', folderNameDate,' is over')); %fixed and added 2/3/2020
        end
    end
