...
classdef autopipeliner_v2a
    methods(Static)
        ...
        function [batchFolder, OGFolder, batches, all] = batches(batches,batchFolder,all,batchCounter,batchFiles) %store all batches to be done
            if nargin < 5
                batchFiles = dir('*.set');
            end
            if nargin < 4
            ...
            for i=1:length(batches)
                ...
                % --- starts pipeline
                [batchFolder,scripts] = autopipeliner_v2a.pipeIn(batches(i),batchFolder); %start the pipeline
        ...
        function [startingFolder,scripts] = pipeIn(scripts,batchFolder) %store the functions to be rolled in this batch
            % --- print
