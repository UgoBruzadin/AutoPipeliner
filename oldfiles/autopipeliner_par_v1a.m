...
classdef autopipeliner_par_v1a
    methods(Static)
        
        %autopipeliner_par_v1a = dbstack;
        %autopipeliner_par_v1a = st.name;
        function folder = getLastFolder(currentDirectory)
            ...
            if size(subFolders,1) > 2
                ...
                %folder = autopipeliner_par_v1a.getLastFolder(folder);
                end
            else
        ...
        function batches(batches,batchFolder) %store all batches to be done
            ...
            for i=1:length(batches)
                ...
                [batchFolder] = autopipeliner_par_v1a.createBatchFolder(OGFolder,files,batchName); 
                
                % --- returns to OG folder
                %cd(OGFolder)
                
                % --- starts pipeline
                [batchFolder] = autopipeliner_par_v1a.pipeIn(batches(i),batchFolder); %start the pipeline
        ...
        function [filesFolder] = pipeIn(scripts,OGFolder) %store the functions to be rolled in this batch
            % --- print
            ...
            filesFolder = OGFolder; %begins with the files inside the main batch folder
            ...
            for i=scriptCounter:length(scripts) %for loop, loops the number of commands
                ...
                [filesFolder] = autopipeliner_par_v1a.Function(OGFolder,scripts(i),filesFolder,scriptCounter);
        ...
        function [filePOST] = Function(OGFolder,script,filesPath,counter) %magic function, runs the code asked!
            if nargin < 4
                counter = 0;
            end
            if nargin < 3
            ...
            autopipeliner_par_v1a.clean(); %wipe the memory
            ...
            [files, filePRE, filePOST] = autopipeliner_par_v1a.createfolders(filesPath,filesPath,folderNameDate,counter); %creates a folder for the pipeline
...
%                 end
            ...
            %autopipeliner_par_v1a.emptyTrash(); %deletes binica's leftover trash
            %sends text to me
            %autopipeliner_par_v1a.txt(strcat('processing of ', folderNameDate,' is over')); %fixed and added 2/3/2020
...
%EEG =  pop_loadset(file1(i).name, FileInput1,  'all','all','all','all','auto');
        ...
        %figuregenerators!
            ...
            for i=1:length(files)
                ...
                %[individualReport] = autopipeliner_par_v1a.tempReport(files(i).name,EEG);%changedneedsfixing
                ...
                autopipeliner_par_v1a.fft(files(i),EEG);
                if ~isempty(EEG.icaweights)
                    autopipeliner_par_v1a.componentFigures(files(i),EEG) %fixed and added 2/3/2020
                end
                EEG = pop_delset(EEG,1); %fixed and added 2/3/2020
            end
            %makes report
            autopipeliner_par_v1a.report(strcat('finalReport',t)); %fixed and added 2/3/2020
            autopipeliner_par_v1a.emptyTrash(); %deletes binica's leftover trash
            autopipeliner_par_v1a.txt(strcat('processing of ', folderNameDate,' is over')); %fixed and added 2/3/2020
        end
    end
