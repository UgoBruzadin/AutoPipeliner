    ...
    methods(Static)
        ...
        function batches(batches,batchFolder) %store all batches to be done
            ...
            for i=1:length(batches)
                cd(WorkingFolder);
                WorkingFolder = autopipeliner.getLastFolder(WorkingFolder);
                cd(WorkingFolder);
                files = dir('*.set');
                ...
                [batchFolder, commandCounter] = autopipeliner.pipeIn(batches(i),batchFolder,commandCounter); %start the pipeline
        ...
        function [filePOST] = Function(batchPath,commands,filePath,counter) %magic function, runs the code asked!
            ...
            parfor i=1:length(files)
                ...
                %saving files & report
                    newname = strcat(files(i).name(1:end-4), [acronym], '.set');
                    EEG = pop_saveset(EEG, 'filename', [newname], 'filepath',filePOST);
                %try autopipeliner.fft(files(i),EEG); catch end
        ...
        function [files, filePRE, filePOST] = createfolders(filePath,batchPath,folderName) %works!!!
            ...
            mkdir (folderName,  'pre'); %created newwe pre folder inside
            ...
            %------
            [setList,fdtList] = autopipeliner.comparefiles(filePRE,filePOST);
